...
static void dump_bytes(scsi_sg_device *dev, const char *prefix, unsigned char *p, int len) {
  int i;
...
int apt_sg16(scsi_sg_device *dev, int rw, int dma, ata_tf_t *tf,
        void *data, unsigned int data_bytes, unsigned int timeout_secs)
{
    sgio_dbgprintf(dev, "APT: SG16 fd %d rw %d dma %d tf %p data %p data_bytes %d timeout %d need SGIO\n",
