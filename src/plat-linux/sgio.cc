 ...
 *
 *            by Mark Lord (C) 2007 -- freely distributable
 */
...
static const int prefer_ata12 = 0;
...
#if 0  /* maybe use this in doTaskfileCmd later.. ? */
...
static int is_dma(__u8 ata_op) {
  switch (ata_op) {
    ...
    case ATA_OP_WRITE_DMA: return SG_DMA;
    default: return SG_PIO;
  }
}

void tf_init(ata::ata_tf_t *tf, __u8 ata_op, __u64 lba, unsigned int nsect) {
...
int sg16(scsi_sg_device *dev, int rw, int dma, ata::ata_tf_t *tf,
  ...
  if (tf->command == ata::ATA_OP_PIDENTIFY)
    prefer12 = 0;

  if (apt_is_apt(dev)) {
    return apt_sg16(dev, rw, dma, tf, data, data_bytes, timeout_secs);
  }
...
int do_drive_cmd(scsi_sg_device *dev, unsigned char *args, unsigned int timeout_secs) {
#ifdef SG_IO
  ...
  rc = sg16(dev, SG_READ, is_dma(tf.command), &tf, data, data_bytes, timeout_secs);
  if (rc == -1) {
    dev->last_errno = errno;
...
int do_taskfile_cmd(scsi_sg_device *dev, struct hdio_taskfile *r, unsigned int timeout_secs) {
  int rc;
#ifdef SG_IO
  ata::ata_tf_t tf;
  void *data = NULL;
  ...
  switch (r->cmd_req) {
    ...
    case TASKFILE_CMD_REQ_RAW_OUT: data_bytes = r->obytes;
      data = r->data;
      rw = SG_WRITE;
      break;
    case TASKFILE_CMD_REQ_IN: data_bytes = r->ibytes;
      data = r->data;
      break;
  }

  rc = sg16(dev, rw, is_dma(tf.command), &tf, data, data_bytes, timeout_secs);
  if (rc == -1) {
    dev->last_errno = errno;
    if (errno == EINVAL || errno == ENODEV || errno == EBADE)
      goto use_legacy_ioctl;
